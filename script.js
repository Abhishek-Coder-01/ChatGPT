    ...
    function addMessageToChat(message, sender) {
      ...
      if (sender === 'user') {
        messageDiv.innerHTML = `
            ...
            <div class="bg-white border border-gray-200 rounded-2xl rounded-br-none px-4 py-3 max-w-md shadow-sm hover:shadow-md transition-shadow duration-200 max-w-3xl">
             <span class="font-semibold text-gray-800">User</span>

            <p class="text-gray-800">${escapeHtml(message)}</p>
            <span class="text-xs text-gray-500 block mt-1 text-right">${timestamp}</span>
          </div>
        `;
      } else {
...
<div class="message-card bg-white border border-gray-200 rounded-2xl rounded-bl-none px-4 py-3 max-w-md shadow-sm hover:shadow-md transition-shadow duration-200 max-w-3xl relative">
    ...
    <div class="flex items-center gap-2 mb-2">
      ...
      <span class="font-semibold text-gray-800">Gemini AI</span>
    </div>

    <div class="text-gray-800 leading-relaxed mb-2 message-content">${formattedMessage.replace(/`/g, "\\`")}</div>
